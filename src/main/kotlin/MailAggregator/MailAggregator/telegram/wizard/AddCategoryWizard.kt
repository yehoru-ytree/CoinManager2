...
class AddCategoryWizard(
    ...
    private val emptyKeywordsTrigger: String by lazy { applyLocale("trigger.emptyKeywords") }
    private val namePattern: Regex by lazy { Regex(applyLocale("validation.namePattern")) }
    private val priorityMin: Int by lazy { applyLocale("validation.priority.min").toInt() }
    private val priorityMax: Int by lazy { applyLocale("validation.priority.max").toInt() }
    ...
    override fun start(context: MessageContext) {
        val promptId = reply(context.msg, applyLocale("addCategory.start", cancelTrigger)) ?: return
    ...
    private fun handleNameStep(chatId: Long, msg: Message, name: String, household: Household) {
        if (!name.matches(namePattern)) {
            val promptId = reply(msg, applyLocale("addCategory.name.bad")) ?: return
            states[chatId] = State.AwaitingName(promptId)
            return
        }
        if (categoryRepository.findByName(household.id, name) != null) {
            val promptId = reply(msg, applyLocale("addCategory.name.exists", name)) ?: return
            states[chatId] = State.AwaitingName(promptId)
            return
        }
        val promptId = reply(msg, applyLocale("addCategory.displayName.prompt")) ?: return
    ...
    private fun handleDisplayNameStep(chatId: Long, msg: Message, displayName: String, prev: State.AwaitingDisplayName) {
        if (displayName.isEmpty()) {
            val promptId = reply(msg, applyLocale("addCategory.displayName.empty")) ?: return
            states[chatId] = prev.copy(lastPromptMessageId = promptId)
            return
        }
        val promptId = reply(msg, applyLocale("addCategory.priority.prompt", priorityMin, priorityMax)) ?: return
    ...
    private fun handlePriorityStep(chatId: Long, msg: Message, priorityRaw: String, prev: State.AwaitingPriority) {
        ...
        if (priority == null || priority !in priorityMin..priorityMax) {
            val promptId = reply(msg, applyLocale("addCategory.priority.bad", priorityMin, priorityMax)) ?: return
            states[chatId] = prev.copy(lastPromptMessageId = promptId)
            return
        }
        val promptId = reply(msg, applyLocale("addCategory.keywords.prompt", emptyKeywordsTrigger)) ?: return
    ...
    private fun handleKeywordsStep(
        ...
        val category = try {
            ...
            reply(msg, applyLocale("addCategory.createFailed", e.message ?: ""))
            return
        }
        states.remove(chatId)
        reply(msg, applyLocale("addCategory.created", category.name, category.displayName, category.sheetRow))
        broadcastInfo(
            household.id,
            excludeChatId = chatId,
            text = applyLocale("addCategory.broadcast", displayNameOf(msg), category.displayName),
        )
    }
