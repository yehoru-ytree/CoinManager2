...
class CashEntryWizard(
    ...
    override fun start(context: MessageContext) {
        val promptId = reply(context.msg, applyLocale("cash.start", cancelTrigger)) ?: return
    ...
    private fun handleAmountStep(chatId: Long, msg: Message, rawText: String, household: Household) {
        ...
        if (amount == null) {
            val promptId = reply(msg, applyLocale("cash.badAmount")) ?: return
        ...
        val tx = try {
            ...
            reply(msg, applyLocale("cash.failed", e.message ?: ""))
            return
        }
        states.remove(chatId)
